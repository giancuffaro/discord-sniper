...
# 9/9: ONE source — market_hours.py owns the holiday table (update it there
# each year). This name stays so bridge.py's import is untouched. The literal
# below is only the fallback if market_hours can't be imported.
try:
    from market_hours import FULL_CLOSE as _FULL_CLOSE
    HOLIDAYS = {d for yr in _FULL_CLOSE.values() for d in yr}
