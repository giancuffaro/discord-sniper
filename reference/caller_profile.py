...
def q(vals):
    """n, median, p25, p75 - or Nones when the sample is empty."""
    ...
    p25 = v[int(round(0.25 * (len(v) - 1)))]
    p75 = v[int(round(0.75 * (len(v) - 1)))]
    return (len(v), med, p25, p75)
...
# ------------------------------------------------------- message classifier
FRAC = re.compile(r"\b(1/2|1/3|1/4|2/3|3/4|1/5)\b|\bhalf\b|\bquarter\b|\bthird\b",
                  re.I)
PARTIAL = re.compile(
    ...
    r"|\bleft\s+a?\s*runner|\bleaving\s+(?:a\s+)?(?:few\s+)?runner|\bruns?ners?\s+left\b"
...
BE = re.compile(r"\bb/?e\b|\bbreak\s*-?\s*even\b|\bbreakeven\b|\bsl\s*(?:to|at|@)\s*b",
                re.I)
LOSS_WORD = re.compile(r"\bstopped\b|\bstop\s+hit\b|\bfor\s+a\s+loss\b|\bloss\b|\bred\b",
                       re.I)
...
def classify(row):
    ...
    if mt == "commentary":
        return "commentary"
    allout = re.search(r"\ball\s*-?\s*out\b|\ball\s+out\s+of\b", t, re.I)
    if (FRAC.search(t) or PARTIAL.search(t)) and not allout:
        return "partial"
    if mt == "trim":
        return "partial"
    if FULL.search(t):
...
def stated_pct(text):
    """The caller's own stated gain/loss percent, or None. Never a size."""
    t = (text or "").replace("\n", " ")
    m = SIGNED.search(t)
    if m:
        return num(m.group(1).replace(" ", ""))
    sizes = set()
    for a, b in SIZE_PCT.findall(t):
        for g in (a, b):
            if g:
                sizes.add(g)
    ...
    toks = ANY_PCT.findall(t)
    toks = [x for x in toks if x not in sizes]
...
# ------------------------------------------------------------ tape (bids)
def load_tape(want):
    """{occ: [(epoch, bid)]} for the OCCs we actually need."""
...
def bid_at(tape, occ, when, tol=120):
    """Bid nearest `when` (a naive ET datetime), within tol seconds."""
    arr = tape.get(occ)
    if not arr:
        return None
...
for e in entries:
    if e["occ"].strip() and linked.get(id(e)):
        need.add(e["occ"].strip())
TAPE = load_tape(need)


# -------------------------------------------------------- per-entry record
def pct_for(m, e):
    """(pct, basis) for one management post."""
    txt = m["source_message_verbatim"]
    p = stated_pct(txt)
    if p is not None:
        return p, "stated"
    ep = num(e["their_price"])
    xp = posted_price(txt)
    if ep and xp and ep > 0:
        if e["kind"] == "future":
            pts = xp - ep
            if e["side"].upper().startswith("S") or "SHORT" in (e["side"] or "").upper():
                pts = -pts
    ...
    occ = e["occ"].strip()
    if occ and m.get("_dt"):
        b = bid_at(TAPE, occ, m["_dt"])
        ep2 = num(e["their_price"])
        if b is not None and ep2 and ep2 > 0 and e["kind"] == "option":
            return (b - ep2) / ep2 * 100.0, "measured"
...
for e in entries:
    ...
    ftp, ftb = pct_for(ft, e) if ft else (None, "")
    fep, feb = pct_for(fe, e) if fe else (None, "")
...
matches = []
for r in OPT:
    if not r["their_price"] and not r["strike"]:
        continue
    st = num(r["strike"])
    for L in led:
        ...
        if not side_eq(L["side"], r["side"]):
            continue
        ls = num(L["strike"])
        if st is None or ls is None or abs(ls - st) > 1e-6:
            continue
        gap = abs(mins(r["e"]["_dt"], L["_o"]))
        if gap > 5:
            continue
        bot_hold = mins(L["_o"], L["_c"]) if L["_c"] else None
        still_in = None
        stopped_first = None
        if r["first_trim_dt"] and L["_c"]:
            still_in = L["_c"] >= r["first_trim_dt"]
            stopped_first = (L["_c"] < r["first_trim_dt"])
        matches.append({
            ...
            "bot_hold_min": bot_hold,
            "bot_pl_pct": num(L["pl_pct"]),
            "bot_source": L["source"],
            "bot_still_in_at_first_trim": still_in,
            "bot_out_before_first_trim": stopped_first,
        })
seen_m = set()
uniq = []
for m in matches:
    k = (m["date"], m["symbol"], m["side"], m["strike"], m["caller"],
         round(m["gap_min"], 3), m["bot_hold_min"])
    if k in seen_m:
        continue
    seen_m.add(k)
    uniq.append(m)
...
for r in sorted(out_rows, key=lambda x: x["_min"]):
    if "trim" not in (r.get("event") or "").lower():
        continue
    k = (r["_day"], r.get("room"), r.get("contract"))
    oc_first.setdefault(k, r)
oc_full = [r for r in out_rows if "full exit" in (r.get("event") or "").lower()]


# --------------------------------------------------------------- 3. stops
posted_opt, posted_fut, underlying_stops = [], [], []
for r in recs:
    if r["their_stop"] is None or not r["their_price"]:
        continue
    s, p = r["their_stop"], r["their_price"]
    if r["kind"] == "future":
        posted_fut.append((abs(s - p), r))
    elif r["kind"] == "option":
        if p <= 0:
            continue
        ratio = s / p
        if 0.2 <= ratio <= 1.0:
            posted_opt.append((abs(s - p) / p * 100.0, r))
        else:
            underlying_stops.append(r)
...
only_pos = [r for r in recs if r["t_first_trim"] is not None
            and (r["exit_pct"] is None or r["exit_pct"] <= (r["first_trim_pct"] or 0))]
...
for name, rs in bycaller.items():
    ...
    if len(lk) < MIN_N:
        continue
    ftm = q([r["t_first_trim"] for r in lk])
    ftp = q([r["first_trim_pct"] for r in lk])
    exm = q([r["t_full_exit"] for r in lk])
    exp = q([r["exit_pct"] for r in lk])
    stp = [abs(r["their_stop"] - r["their_price"]) / r["their_price"] * 100.0
           for r in rs
           if r["their_stop"] and r["their_price"] and r["kind"] == "option"
           and 0.2 <= r["their_stop"] / r["their_price"] <= 1.0]
    ...
    prof.append({
        ...
        "entries_all": len(rs),
        "entries_linked": len(lk),
        "median_min_to_first_trim": ftm[1], "p25_min_first_trim": ftm[2],
        "p75_min_first_trim": ftm[3], "n_first_trim": ftm[0],
        "median_first_trim_pct": ftp[1], "n_first_trim_pct": ftp[0],
        "median_min_to_exit": exm[1], "n_exit": exm[0],
        "median_exit_pct": exp[1], "n_exit_pct": exp[0],
        "posts_a_stop_pct": 100.0 * posted_n / len(rs),
        "median_stop_pct_of_premium": q(stp)[1], "n_stop_pct": len(stp),
...
def score(p):
    """matchable = a first trim that exists, at a reachable % and a real clock."""
    if not p["n_first_trim"] or p["median_first_trim_pct"] is None:
        return -1.0
    s = min(p["n_first_trim"], 40) * 1.0
    if p["median_min_to_first_trim"] is not None and p["median_min_to_first_trim"] >= 1.0:
        s += 20
...
# ----------------------------------------------------------------- output
def block(title, rows_opt, rows_fut):
    out = ["### %s" % title, ""]
    for lab, rs in (("Options", rows_opt), ("Futures", rows_fut)):
        n = len(rs)
        ft = q([r["t_first_trim"] for r in rs])
        ex = q([r["t_full_exit"] for r in rs])
        sil = sum(1 for r in rs if r["silent"])
        nox = sum(1 for r in rs if r["no_exit"])
        out += ["**%s** - %d entries (high+medium confidence, %s .. %s)" %
                (lab, n, WIN_LO, WIN_HI), "",
                "| measure | value |", "|---|---|",
                "| entry -> first trim (min) | %s |" % qs(ft),
                "| entry -> full exit (min) | %s |" % qs(ex),
...
A("")
A("Question, in G's words: *\"what is the callers' average trade hold time, "
  "because we need to kind of match that to get similar results - I'm even happy "
  "getting to the first trim. What's their normal stop also?\"*")
A("")
...
A("")
A("**The linking caveat, stated up front.** %d management messages in the whole "
  "file carry no `links_to` at all - they name a ticker and a percent and nothing "
  "else, so they can never be tied to an entry. A further %d linked messages point "
  "at an entry that is outside this window or below medium confidence, %d are "
  "ambiguous (two entries share the timestamp), and %d timestamp *before* their "
  "entry. All of those are excluded. **Every hold-time number below is therefore "
  "measured on the callers who write a linkable management post - the tidy ones. "
  "It is not a sample of all calls.**" % (UNLINKABLE, drop_noentry, drop_ambig,
                                          drop_backwards))
A("")
A("---")
A("")
A("## 1. Hold time")
A("")
L += block("Overall", OPT, FUT)
A("Read the silent line carefully: an entry with no posted exit is not a loss and "
  "not a win. It is **unavailable**. A later high in the tape is not a caller exit.")
A("")
A("---")
A("")
A("## 2. The first trim")
A("")
ftm = q([r["t_first_trim"] for r in recs])
ftp_all = q([r["first_trim_pct"] for r in recs])
bas = Counter(r["first_trim_basis"] for r in recs if r["t_first_trim"] is not None)
A("| measure | value |")
A("|---|---|")
A("| entries that got a first trim | %d of %d |"
  % (sum(1 for r in recs if r["t_first_trim"] is not None), len(recs)))
A("| minutes from entry to it | %s |" % qs(ftm))
A("| percent it is posted at | %s |" % qs(ftp_all, 1, "%"))
A("| where that percent came from | %s |"
  % ", ".join("%s %d" % (k or "none", v) for k, v in bas.most_common()))
A("| first trim is the only positive event (no better exit recorded) | %d of %d |"
  % (len(only_pos), sum(1 for r in recs if r["t_first_trim"] is not None)))
A("")
...
A("### (a) Posted stops")
A("")
n_stop = sum(1 for r in recs if r["their_stop"] is not None)
A("| measure | value |")
A("|---|---|")
A("| entries carrying a posted stop | %d of %d = %s |"
  % (n_stop, len(recs), fmt(100.0 * n_stop / len(recs) if recs else None, 1, "%")))
A("| option stops quoted as a premium, distance below entry | %s |"
  % qs(q([x for x, _ in posted_opt]), 1, "%"))
A("| option \"stops\" that are an UNDERLYING level, not a premium | %d - excluded |"
  % len(underlying_stops))
A("| futures stops, points from entry | %s |" % qs(q([x for x, _ in posted_fut]), 1))
A("")
futbot = sum(1 for _, r in posted_fut if "NEW POTENTIAL SIGNAL" in
             (r["e"]["source_message_verbatim"] or ""))
if futbot:
    A("Of the futures stops, **%d come from one automated signal poster** that "
      "prints a fixed `TP:/SL:` on every message. That is a machine's parameter, "
      "not a human's risk decision - read it separately." % futbot)
    A("")
A("### (b) Revealed stops - where a loss exit was actually posted")
A("")
A("| measure | value |")
A("|---|---|")
A("| entries with a posted losing exit and a readable percent | %d |" % len(revealed))
A("| the loss at that exit | %s |" % qs(q([r["exit_pct"] for r in revealed]), 1, "%"))
A("")
A("### (c) \"SL to b/e\"")
A("")
A("| measure | value |")
...
A("| minutes after entry that happens | %s |" % qs(q([r["be_min"] for r in be_rows])))
A("")
A("---")
A("")
A("## 4. Runner vs trim")
...
A("")
A("Join: same date, same symbol, same strike, same side, our entry within 5 "
  "minutes of theirs, against `master_ledger.csv`.")
A("")
A("| measure | value |")
A("|---|---|")
...
A("")
if both:
    A("Side by side, every answerable pair:")
    A("")
...
A("`daily-reports/CALLER-OUTCOMES-*.csv` pairs claim events by hand for two days. "
  "Small, but every row has both timestamps.")
A("")
tt = q([r["_min"] for r in oc_first.values()])
tf = q([r["_min"] for r in oc_full])
A("| measure | value |")
A("|---|---|")
A("| claim events paired across the two days | %d |" % len(out_rows))
A("| distinct positions whose FIRST trim is timed | %d |" % len(oc_first))
A("| entry -> first trim (min) | %s |" % qs(tt))
A("| first-trim %% | %s |"
  % qs(q([r["_pct"] for r in oc_first.values()]), 1, "%"))
A("| full exits recorded | %d |" % len(oc_full))
A("| entry -> full exit (min) | %s |" % qs(tf))
A("")
mb = sum(1 for r in out_rows if "market bid" in (r.get("basis") or ""))
A("Basis on those rows: %d caller-stated, %d market-bid-at-exit (measured), "
  "%d unavailable." % (sum(1 for r in out_rows if (r.get("basis") or "") == "caller-stated"),
                       mb,
...
A("")
A("| caller | n entries / linked | median min to 1st trim | typical 1st-trim % | "
  "posts a stop | typical stop % of premium | silent | median exit % | matchable |")
A("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
for p in prof:
    A("| %s | %d / %d | %s | %s | %s | %s | %s | %s | %s |" % (
        p["caller"][:24], p["entries_all"], p["entries_linked"],
        fmt(p["median_min_to_first_trim"]) + (" (n=%d)" % p["n_first_trim"]),
        fmt(p["median_first_trim_pct"], 1, "%") + (" (n=%d)" % p["n_first_trim_pct"]),
        fmt(p["posts_a_stop_pct"], 0, "%"),
        fmt(p["median_stop_pct_of_premium"], 1, "%") + (" (n=%d)" % p["n_stop_pct"]),
...
A("| room | entries | linked | median min to 1st trim | median 1st-trim % | "
  "median min to exit | silent |")
A("|---|---:|---:|---:|---:|---:|---:|")
for room, rs in sorted(byroom.items(), key=lambda kv: -len(kv[1])):
    ...
    if len(lk) < MIN_N:
        continue
    A("| %s | %d | %d | %s | %s | %s | %s |" % (
        room[:44], len(rs), len(lk),
        fmt(q([r["t_first_trim"] for r in rs])[1]),
        fmt(q([r["first_trim_pct"] for r in rs])[1], 1, "%"),
        fmt(q([r["t_full_exit"] for r in rs])[1]),
        fmt(100.0 * sum(1 for r in rs if r["silent"]) / len(rs), 0, "%")))
A("")
A("## 9. What this cannot say")
A("")
A("- It cannot give a caller's win rate or net result. Most entries never get a "
  "posted exit, and an unposted exit is unavailable, not a number.")
A("- The hold times describe **callers who post management messages the recovery "
  "could link**. Rooms that post an entry and go quiet are in the silent column, "
  "not in the medians.")
A("- `their_target` is not recorded anywhere in this repo, so no target study is "
  "possible.")
A("- Percentages labelled `stated` are the caller's own claim. They are not "
  "broker truth and they are not the bid we could have hit.")
A("")
...
cols = ["caller", "room", "kind", "entries_all", "entries_linked",
        "median_min_to_first_trim", "p25_min_first_trim", "p75_min_first_trim",
        "n_first_trim", "median_first_trim_pct", "n_first_trim_pct",
        "median_min_to_exit", "n_exit", "median_exit_pct", "n_exit_pct",
        "posts_a_stop_pct", "median_stop_pct_of_premium", "n_stop_pct",
        "silent_rate_pct", "no_posted_exit_pct", "matchable_score"]
...
print(json.dumps({
    ...
    "matches": len(matches), "answerable_pairs": len(st_in) + len(st_out),
    "bot_still_in": len(st_in), "bot_out_first": len(st_out),
}, indent=1))
