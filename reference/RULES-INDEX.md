...
| 71 | 55 | a Webull futures OPEN refuses until `futures_protection_proof.json` proves the fill→stop→verify→cancel loop at the broker; the proof dies when webull_futures.py changes | ENFORCED | webull_futures.py `protection_proof_state()` (steps + sha256 of the module) gates `execute()`'s OPEN with the reason in the refusal; bridge.py /mode ships `webull_futures_entry_ready` + `_reason`; the proof is written only by `futures_protection_proof.py --live` (G's own one-lot MES trade) |
