...
| 71 | 55 | a Webull futures OPEN refuses until `futures_protection_proof.json` proves the fill→stop→verify→cancel loop at the broker FOR THAT MICRO; MES proven is not MNQ proven, and both proofs die when webull_futures.py changes | ENFORCED | webull_futures.py `protection_proof_state(symbol)` (that symbol's block: steps + sha256 of the module) gates `execute()`'s OPEN with the reason in the refusal, naming the symbol; ES/NQ are judged on the micro they map to; bridge.py /mode ships `webull_futures_entry_by_symbol` plus the both-micros summary; a block is written only by `futures_protection_proof.py --symbol <MES|MNQ> --live` (G's own one-lot trade) and merges without touching the other |
