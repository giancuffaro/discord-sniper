...
Last updated 9/7/26 (greeks stream removed — see the box below).
...
G's own tool, **127.0.0.1:8000** (futures at /futures, same server). His **manual** scalps
...
> **9/7 — THE DXLINK GREEKS STREAM LEFT THIS APP** (HANDOFF-REMOVE-GREEKS,
> at G's instruction: "remove the greeks then, we are only going to use it
> here in discord sniper"). tastytrade caps concurrent DXLink sessions per
> account; both apps were on the same account, at the cap, and the bridge
> logged `dxlink refused RE-AUTH: The number of user sessions has exceeded
> the configured limit` three times on 9/7. `dxlink.py` is deleted here;
> nothing in this folder can construct a session. Option quotes for open
> contracts now come from **Tradier**, one batched request a second for
> every watched contract (`marketdata.Feed.option_quote`, hooked into
> `webull_client.OPTION_QUOTE_HOOK` when the feed is built). tastytrade REST
> stays as the stock-price fallback; its credentials stay in
> `my-settings.json`. **Standing rule: never open a tastytrade DXLink
> session against this account from this folder — no probes, no scratch
> scripts.** Scenarios 68 and 80 lock it in.

**The market-data work is finished and finished well — do not redo it.**
`marketdata.py` wraps Tradier + tastytrade REST (Tradier for stock AND
option quotes; tastytrade's REST market data 403s on this account, so it is
the fallback only). `my-settings.json` has `feeds.tasty_client_secret`,
`feeds.tasty_refresh_token` and `feeds.tradier_token` set. `main.py`'s price
path is **free feeds → Webull → Yahoo**, Webull demoted. `/api/feeds/test` reports the
...
* ~~`stop_below()`~~ — not applicable here and removed (9/6). Market Sniper
  rests NO stop at the broker; every exit is app-managed. Keep it in the bot.
...
**Greeks:** NOT from this app (9/7, box above) — discord-sniper owns the
DXLink session and stamps greeks on its own fills. If greeks are ever
wanted in this log, ask discord-sniper for them or compute from IV via
Black-Scholes (`gauges.bs_delta` already does). The notes below are what
was measured before the stream moved out and still hold for the bridge.
Subscribe each open contract, stamp `greeks_in` at entry and `greeks_out`
at exit. First greeks arrive **0.06s** after subscribing.
