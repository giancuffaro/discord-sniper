...
| Who's profitable? / scoreboard | `SCOREBOARD.html` (`python reports.py build scoreboard` if `status` says STALE) | the per-room table; money comes from master_ledger.csv only | not the announcer board, not counts from DS Logs |
