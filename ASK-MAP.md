...
| Best stop / arm / rung / target per instrument (MES vs MNQ), 520 combos, both-halves check | `reference/RATCHET-SWEEP.txt` (rebuild `python reference/ratchet_sweep.py`) | TOP 12, BOTTOM 3, BEST positive in both halves, knob averages | measurement only; n=64/40 — read the overfit line first |
