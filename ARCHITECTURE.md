...
| `futures_protection_proof.py` | 824 | the supervised one-lot run that writes that proof, one micro at a time (`--symbol MES` / `--symbol MNQ`). Never called by the bridge |
