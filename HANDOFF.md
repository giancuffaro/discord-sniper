...
Last updated: 2026-09-15 — cut to a RULES CORE: one line per rule, every mechanic moved verbatim to its reference doc (28,060 bytes before). Ceiling is now 14 KB, not 30.
...
- ONE RULE, ONE LINE: `- NAME (G, date): the imperative. reference/DOC.md`. Numbers, formats, procedures, examples, history and rationale are MECHANICS.
...
rules · ARCHITECTURE.md the machine paragraph, accounts, coexistence and the
north star · DATA-MAP.md the CONDENSE AND MERGE test · reference/
EOD-BENCHMARK-SPEC.md the report contract · reference/PULLBACK-LEVELS.md ·
reference/CALLER-LEDGER.md · HANDOFF-LOG.md (history) · extension/rooms.txt ·
settings.json (keys, gitignored) · master_ledger.csv / master_alerts.csv
(truth) · days/ · daily-reports/ + daily-audits/ (one file per week per kind).
...
- AI READS ARE PROPOSALS: the parser and every guard judge them, AI confidence authorizes nothing; "" or a range is NO CALL — not a crash, not an order; department output is advisory, never code or orders.
...
- PRICE: the caller's price or better; pullback entries cross the ask; one contract per entry.
- RN PULLBACK is global and ON; THE LEVEL STAYS $1 — SETTLED 9/9, never re-opened on a feeling.
- ONE SWITCH PER ROOM (G, 9/9): ON = tab + read + trades LIVE, OFF = nothing, LAPSED = the sub ran out. No paper/testing state. CLOSING A TAB BY HAND IS NOT A BENCH; benched rooms are never deleted.
- TABS (9/10): the reaper closes only `_OURS`, never a human's; only START HERE, the popup switch and whopSelfHeal() may open one; "No Access" → `lapsed` + close.
- ROOM RULES = rooms.txt's 6th field, set from the popup; settings.json holds none; `spx` DELETED 9/10. Hours 9:15–16:30 ET weekdays unless `always`; a hand-closed tab stays closed; the last browser tab is never closed.
- CHANNELS: callers shown inside their verified room, win rate unavailable without evidence; no separate Callers / Needs You tab (G).
- STRIKES: never more than 1 strike OTM; 3-ITM for SPY/QQQ/Mag7 0DTE; ADD buys the held strike. "ADDED <full contract>" you are not in = an OPEN entry; a bare "added to SPY" refuses.
- NO SPX→SPY (G, 9/10: "do not translate any SPX to SPY"); index entries are HELD until execution.index_broker is set.
- WORD ORDER: any order, `bare` rooms only. TWO CONTRACTS = TWO ORDERS (9/10): one each, own born stop and ratchet, same ticker only; call+put refuses the line.
- EXPIRY in ONE place: NDTE = N CALENDAR days rolling BACK, never past today; NO DATE = 0DTE (G, 9/10) with the LISTING ASKED, never assumed; "FRIDAY WEEKLIES ONLY" is DEAD.
- THE CONTRACT MUST EXIST (9/15) AND BE IN PRICE BAND (9/14): siblings listed → REFUSE, nothing listed → THROUGH with a LISTING line; fails open, a guard never a gate.
- GUARDS: SPREAD and THIN refuse wide or illiquid entries; nothing older than 3 min fires; negations hard-veto; DEDUPE ends at ONE average-down ADD under what was PAID; identity = caller+symbol+strike+side+expiry.
...
- FUTURES: micros only, their stop/target wins, a Webull futures OPEN refuses until an exact GTC STOP_LOSS is verified after its fill. INDEX MIRROR (9/13) OFF until a broker-confirmed futures protective exit exists. THE POCKET default OFF.
- REVIEWS (9/13): the optionality channel is ON by G's choice; no arbitrary premium range, no automatic factor-of-100 correction.
...
- THE RATCHET (5/3/5 since 9/10, flat): born stop −5%, +3% → breakeven, each further +5% locks +5%; ratchet_tiers.py is the one implementation; stops never loosen; anti-clip off.
- FUTURES RATCHET (9/9) comes from the trade's own risk, never a fixed number.
- SWINGS (14+ DTE, auto-tagged): their stock-level stop runs it, no level = wide −25% re-armed at 9:31; scalps excluded on purpose.
- CLOSE path: every bot sell waits for FILLED; a CLOSE for a contract the book does not hold is REFUSED, never sent.
...
- BOOT: state photo every event; everything UNVERIFIED until the broker confirms; expired options = dead paper; updates self-apply at the first safe window. POSTCHECK logs "POSTCHECK … PROBLEM" when book, stop and quote bus disagree.
...
- WEEKLY REPORTS (9/15): ONE file per week per kind, newest day first, a re-run replacing that day's block; reports.py owns it, writers never mint a dated file.
- APPEND, DON'T PILE (G, 9/15). New data goes INTO the one living file for its kind — never a new dated file beside it; rotated logs and finished experiments go zipped to `archive/`.
...
- THE 16:40 AUDIT: broker-confirmed actuals override any simulation; RAW capture is kept and LIVE PARSER rows overlay it; relay duplicates count once; expired pullback waits are skips; the deleted 15-minute Codex guard is never recreated.
- GIT: settings.json holds every key and is never committed; AUTO PUSH owns commits (every 45 s); never run git write commands from a sandbox.
...
- READ DATA-MAP.md WITH INDEX.md every session: INDEX.md says what a file IS, DATA-MAP.md what is IN it. RUN build_ledger.py IN EASTERN.
- COMPILE-CHECK everything touched (py_compile / node --check); extension changes bump extension/manifest.json; never install webullsdkcore into the bridge's Python; sandbox trading is RETIRED, paper is LOCAL (SIM tickets).
- DISCORD API IS NOT AN OPTION (9/9): user-token automation risks a permanent ban on the account and the paid subs; official bots need the server owner. Browser reads only.

ROOMS / TABS / READERS (mechanics: reference/ROOMS-TABS.md)
- rooms.txt = THE channel list (tabs + trading, one file). START HERE IS FULLY UNATTENDED (G, 9/9); between runs NOTHING opens rooms; the only inputs no script may do are a Discord/Whop login and Webull keys in the popup.
- Relays: OWLS all-alerts is active, RELAY UNWRAP re-books under the real trader; ZTRADEZ, shabs and eli direct rooms retired 9/9. TAB HEALTH: never close a human-owned tab; Profile 2 = Discord, Profile 6 = Whop.
- VOICE: ears transcribe always (Deepgram); voice ENTRIES ON (9/2), voice EXITS irrelevant under entries-only; a typed copy of a voice fire is an echo. A room quiet 40 min in market hours raises the silence alarm.
...
- PAUSED since 9/2. Its board is computed FROM THE LEDGER (9/9); its order hunt is paced — the 9/2 429 storm must never come back.

## DATA — one central file per family (9/9). THE APP READS ONLY THESE.
(what is inside each: DATA-MAP.md)
- BROKER RECORD → master_broker.csv; the Webull export is ONE fixed file OVERWRITTEN every run — never dated piles; one balance row per day in balance_daily.csv.
- FILLS → master_ledger.csv. The broker's exit/P&L/state/account WIN over the book's belief; a DRIFT line means something upstream lied; nothing reads days/*.json "table" or journal.csv for analysis.
...
- BOT ATTRIBUTION: a caller name is candidate evidence until the entry links to an alert and the trade to broker fills; never quote P&L from book-priced rows when a broker row exists.

## Broker facts — RULES only (all of them: reference/OPTIONS-BROKER-REFERENCE.md, before any broker test)
...
- THE PAGE is the SAME popup.html opened as a normal tab — never a second dashboard. Claude-in-Chrome CANNOT read the popup; the red line under the rooms is the diagnosis — ask for it.
