...
Last updated: 2026-09-16 — the futures door now opens on EVIDENCE: `futures_protection_proof.json` (PROVE FUTURES STOPS.bat, his own one-lot MES trade), never a hardcoded False. Discord tabs: weekday 8:55 START HERE task. Ratchet 10/10/10 since 9/15.
...
- FUTURES: micros only; their stop/target wins; a Webull futures OPEN refuses until `futures_protection_proof.json` proves the fill→stop→verify→cancel loop AT THE BROKER — the proof carries webull_futures.py's sha256 and DIES the moment that file changes, and running it is HIS own one-lot MES trade (`PROVE FUTURES STOPS.bat`; dry by default, `--live` + typed YES sends). INDEX MIRROR (9/13) OFF until a broker-confirmed futures exit exists; THE POCKET default OFF.
