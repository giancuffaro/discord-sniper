...
def render(out=OUT):
    ...
    for r in db.execute("SELECT * FROM trade_attribution WHERE kind='reconciled_trade' ORDER BY date DESC,record_id"):
        ...
        trades.append([r['date'],r['attribution_name'],r['room'],r['symbol'],p.get('strike'),p.get('expiry'),
            p.get('fill'),p.get('exit_avg'),p.get('pl'),r['entry_basis'],r['exit_method'],r['identity_status'],r['resolved_trader_id'],
            r['candidate_trader_id'],r['name_basis'],r['origin_classification'],
            ', '.join('trades.log:'+str(e['order_line'])+'/'+str(e['caller_line']) for e in json.loads(r['entry_evidence'] or '[]')),
            r['source_path'],r['source_row']])
