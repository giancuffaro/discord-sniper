...
def build_book():
    ...
    # ANTI-CLIP off unless he turns it on (9/4). Plain ladder only, so the
    # next weeks of recorded trades are one clean sample instead of two.
    BOOK.anticlip = bool((CFG.get("strategy") or {}).get("anticlip", False))
    ...
    if BOOK.ratchet_on:
        ...
        note("STRATEGY forced ON at bridge start: 1 contract, -%.1f%% stop to "
             "start, then at +%.1f%% the stop goes to %s and every "
             "further +%.1f%% locks another %.1f%% — "
...
def _expiry_recently_past(exp):
    ...
    try:
        ...
        for fmt in ("%m/%d/%Y", "%m/%d/%y", "%m/%d"):
            try:
                if fmt == "%m/%d":
                    # Python 3.15 will stop guessing a year for "m/d"
                    # (DeprecationWarning today): pin this year explicitly;
                    # the roll-forward logic below still fixes January.
                    m = datetime.strptime("%s/%d" % (s, eastern.now().year), "%m/%d/%Y")
                else:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _ai_read(self):
        ...
        # judge() = validate() that can never raise; see ai_reader.judge.
        ok, why, cleaned = ai_reader.judge(read, text, allowed)
    ...
    def _ai_read_image(self):
        ...
        check_text = (caption + "\n" + seen).strip()
        # judge() = validate() that can never raise; see ai_reader.judge.
        ok, why, cleaned = ai_reader.judge(read, check_text, allowed)
        if not ok:
        ...
        if isinstance(body.get("strategy"), dict):
            ...
            if BOOK is not None and BOOK.ratchet_on:
                ...
                note("STRATEGY ON: 1 contract, -%.1f%% stop to start, then at "
                     "+%.1f%% the stop goes to %s and every further "
                     "+%.1f%% locks another %.1f%%"
