...
VERDICT — 0 no-fills today; nothing beats today's rule: every slack level from 2% to 10% comes out behind it (-$13 to -$244), and the 95% band on 158 paired orders (-$2.61 .. -$0.56 per order) CLEARS zero

## The population

| | n | note |
|---|---:|---|
| option ORDER IN lines, all time | 224 | every option order the bot has ever sent |
| with a recorded outcome | 201 | a FILLED or NOFILL line after it; the rest were cancelled, edited or never resolved |
| never filled | 34 | the 90-second window expired with the bid unhit — the population this rule is about |
| filled | 167 | where the cost side lives |
...
| fills SCORED | 148 of 167 | the cost is only charged where a real ask was recorded |

COVERAGE, said plainly: **10 of the 34 option no-fills can be scored**. 7 are quarantined and 17 have no quote at read time. The benefit side of this question rests on 10 trades; the cost side on 148. They are not measured to the same standard and the table below must be read that way.

`grep -c NOFILL trades.log` says 52. Five of those are POSTCHECK lines about a no-fill; 20 of the remaining 47 are FUTURES (16 MNQ, 4 MGC), which have no ask to cross on this rule. The option population is 34 of 224 ORDER INs (15%), not 52 of 204.
...
|---|---|---|---|---|---|---|
| 0% — today's rule | 0 of 10 | +$0 | 120 of 148 | -$386 | -$386 | **baseline** |
| 2% | 0 of 10 | +$0 | 129 of 148 | -$399 | -$399 | **-$13** |
| 3% | 0 of 10 | +$0 | 133 of 148 | -$436 | -$436 | **-$50** |
| 5% | 3 of 10 | +$27 | 142 of 148 | -$528 | -$501 | **-$115** |
| 7.5% | 7 of 10 | -$21 | 147 of 148 | -$642 | -$663 | **-$277** |
| 10% | 10 of 10 | +$17 | 148 of 148 | -$647 | -$630 | **-$244** |

**Read the last column, not the one before it.** The model prices a cross at the recorded ask, and our real fills came in BETTER than that ask — so the slack-0 column is already -$386 against the broker's own prices on 120 of 148 scored fills. That bias is the same at every slack level, so it cancels in the difference and poisons the absolute.

Across ALL 167 filled orders in the record — scored or not — resting at the caller's price filled **+$1316** better than the price we bid, about +$0.08 a contract. That is the thing crossing spends.
...
|---|---|---|---|---|---|
| 2% | 158 | -$0.08 | -$0.15 .. -$0.03 | 0% | real at this sample — and it is on the **LOSS** side |
| 3% | 158 | -$0.32 | -$0.65 .. -$0.09 | 0% | real at this sample — and it is on the **LOSS** side |
| 5% | 158 | -$0.73 | -$1.35 .. -$0.15 | 1% | real at this sample — and it is on the **LOSS** side |
| 7.5% | 158 | -$1.75 | -$2.78 .. -$0.82 | 0% | real at this sample — and it is on the **LOSS** side |
| 10% | 158 | -$1.54 | -$2.61 .. -$0.56 | 0% | real at this sample — and it is on the **LOSS** side |
...
One asymmetry matters more than the band: the 148 fills give the cost side a real sample, while the benefit side has 10 trades. So a band that clears zero here is evidence about the COST of crossing, not proof about its upside.

## Today's no-fills (2026-09-18)
...
- 158 scored orders cannot settle a trading rule. They can rule things out, and they can price a cost.
