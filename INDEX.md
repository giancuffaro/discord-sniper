...
| `PROVE FUTURES STOPS.bat` | **The two real trades that open futures.** Runs `futures_protection_proof.py --symbol all` in DRY RUN (preflight + a plan per micro, nothing sent) and prints the exact two commands to run for real: `--symbol MES --live` ($50 of risk) and `--symbol MNQ --live` ($20). A live run places ONE real order in that micro, proves the fill → stop → verify → cancel → flat loop at the broker and writes that micro's block of `futures_protection_proof.json`. It never passes `--live` for him. |
...
| `entry_slack.py` | The ONE crossing rule: given the caller's price and the contract's real bid/ask, would we cross the ask? `decide()` is pure (no I/O, legal ticks only) and shared with the replay. One switch — `execution.entry_slack_pct`, **0 and activation BLOCKED**: a non-zero value refuses to arm and logs why. Measured, not argued — `reference/entry_slack_replay.py`. |
...
| `futures_protection_proof.py` | The harness behind that .bat, and the only thing that can open the Webull futures entry gate. ONE contract of ONE micro per run (`--symbol MES` / `MNQ` / `all`), every step verified at the broker, loud and specific on every failure (which client order id to look up, what to close by hand), no retries, and no proof block unless every step passed. Proving one micro MERGES into the proof file and leaves the other's block untouched. Reading the gate: `webull_futures.protection_proof_state(symbol)`. |
...
| `broker_sync.py` | **The broker pull that never existed.** First step of the 16:40 audit: one read-only WebullOptions, the day's order history (paged) → `Webull_Orders_auto.csv` OVERWRITTEN, one balance read → `balance_daily.csv`, the FUTURES account's filled legs + fees → `master_futures.csv` and its NLV / net / fees / money-moved `flow` onto the same balance row (9/16), then `build_ledger`. Until 9/15 nothing pulled it — a Claude session did it by hand, last on 9/11, so 9/12–9/14 reported "broker export missing". |
...
| `reference/entry_slack_replay.py` | Every evening (called by `daily_audit.py` through `reports.py` after the audit): replays every option no-fill in `trades.log` at slack 0/2/3/5/7.5/10% on the real recorded bid/ask, runs the rescues forward under the LIVE ratchet, and subtracts the price improvement crossing would give away on the orders that DID fill → that day's block in `daily-reports/ENTRY-SLACK week-of-….md` + `reference/ENTRY-SLACK-REPLAY.csv`. 9/15 baseline: 27 option no-fills of 201 orders, 3 scorable; every slack level loses (−$13 to −$291), band clears zero on the LOSS side. |
| `test_positions.py`, `test_architecture.py`, `test_brokers.py`, `test_phantom_exit.py`, `test_tape.py`, `test_alert_tape.py`, `test_expiry.py`, `test_index_mirror.py`, `test_entry_slack.py`, `test_resolve.js`, `extension/test_*.js` | The suite. `test_expiry.py` holds every date shape the rooms actually write — a date this reader can't take is not a skipped trade, it's this Friday bought silently. |
